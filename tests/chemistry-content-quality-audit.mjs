...
const report={files:files.length,questions:0,formats:{},mc:0,calculations:0,duplicateIds:[],duplicateOptions:[],missingCorrectOption:[],calculationSchema:[],conceptCounts:{},notationCandidates:[],byFile:{}};
const ids=new Map();

for(const file of files){
  const doc=JSON.parse(fs.readFileSync(path.join(root,file),"utf8"));
  const qs=doc.questions||[];
  const stat={questions:qs.length,formats:{},mc:0,calculations:0};
  ...
  for(const q of qs){
    ...
    const accepted=(q.answer?.accepted||[]).filter(x=>typeof x==="string");
    const meta=(q.prompt||"")+" "+(q.task?.label||"")+" "+(q.task?.instruction||"")+" "+(q.stimulus?.text||"");
    for(const ans of accepted){
      const compact=ans.trim();
      const intent=/formula|symbol|equation|ion|charge|oxidation state/i.test(meta);
      const notation=/^[0-9\s+\-()=><→⇌.·]*[A-Za-z][A-Za-z0-9₀-₉\s+\-()=><→⇌.·]*$/.test(compact)&&compact.length<=60;
      if(intent&&notation) report.notationCandidates.push({file,id:q.id,format:q.format,prompt:q.prompt,answer:compact});
    }
  }
...
const summary={
  ...
  repeatedConcepts,maxConceptMultiplicity,notationCandidates:report.notationCandidates.length
};
console.log("CHEMISTRY_QUALITY_SUMMARY "+JSON.stringify(summary));
console.log("CHEMISTRY_CALC_SCHEMA "+JSON.stringify(report.calculationSchema.slice(0,100)));
console.log("CHEMISTRY_BAD_MC "+JSON.stringify({duplicateOptions:report.duplicateOptions.slice(0,80),missingCorrectOption:report.missingCorrectOption.slice(0,80)}));
console.log("CHEMISTRY_NOTATION_CANDIDATES "+JSON.stringify(report.notationCandidates.slice(0,120)));
fs.mkdirSync("test-results",{recursive:true});
fs.writeFileSync("test-results/chemistry-content-quality.json",JSON.stringify(report,null,2));
