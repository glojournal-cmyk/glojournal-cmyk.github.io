...
for(const file of html){
  ...
  rows.push({file:rel,main:src.includes(mainModule),refs:refs.length});
}
...
const summary={htmlShells:html.length,failures:failures.length,rootEquals404:rootHtml===notFound,manifest:{name:manifest.name,start_url:manifest.start_url,scope:manifest.scope,display:manifest.display}};
