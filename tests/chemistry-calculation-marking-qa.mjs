...
for(const q of calcs){
 ...
 if(unit){
   ...
   for(const alias of a.units||[]){
     const ar=B(q,value+" "+alias,"chemistry",{});
 ...
 if(value!==0&&Math.abs(value)>=1000){
   ...
   r=B(q,input,"chemistry",{});
   if(!r.ok) failures.push({type:"scientific-notation-rejected",file:q.__file,id:q.id,input,answer:a,result:r});
 }
 if(Number.isInteger(value)&&Math.abs(value)>=1000){
   const input=value.toLocaleString("en-GB")+(unit?" "+unit:"");
   r=B(q,input,"chemistry",{});
   if(!r.ok) failures.push({type:"thousands-separator-rejected",file:q.__file,id:q.id,input,answer:a,result:r});
 }
}
...
console.log("CHEMISTRY_CALC_MARKING_SUMMARY "+JSON.stringify(summary));
console.log("CHEMISTRY_CALC_MARKING_FAILURES "+JSON.stringify(failures.slice(0,150)));
fs.mkdirSync("test-results",{recursive:true});
fs.writeFileSync("test-results/chemistry-calculation-marking.json",JSON.stringify({summary,failures},null,2));
