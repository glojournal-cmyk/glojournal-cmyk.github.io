...
const report = {
  ...
  obviousMcKeyMismatch: {}, missingScorableAnswer: {}, schema: {}
};
const seen = new Map();
...
for (const file of files) {
  ...
  for (const q of qs) {
    ...
    if(q.format==="mc_single"){
      if(!accepted.length){
        const item={file,id,prompt:q.prompt,options};
        add(report.mcMissingAccepted,subject,item); report.fatal.push({subject,file,id,type:"mc_missing_accepted"});
      } else {
        ...
        const mentioned=options.filter(o=>positive.includes(o));
        const uniq=[...new Set(mentioned)];
        if(uniq.length===1 && !accepted.some(v=>norm(v)===norm(uniq[0]))){
          const item={file,id,accepted,feedbackCandidate:uniq[0],options};
    ...
    if(exactFormats.has(q.format) && q.format!=="mc_single"){
      if(!accepted.length && !a.modelAnswer && !Array.isArray(a.requiredGroups)){
    ...
    if(q.format==="calculation"){
      if(typeof a.value!=="number" || !Array.isArray(a.units)){
        report.fatal.push({subject,file,id,type:"bad_calculation_schema",value:a.value,units:a.units});
      }
    }
