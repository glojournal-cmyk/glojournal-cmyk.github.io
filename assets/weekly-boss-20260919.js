...
function WeeklyBoss(){
 ...
 french&&phase===3?(0,G.jsxs)(G.Fragment,{children:[(0,G.jsx)(`p`,{className:`mt-5 font-medium`,children:task.prompt}),(0,G.jsx)(`div`,{className:`mt-3 min-h-14 rounded-xl border border-line bg-sage/25 p-3 text-sm`,children:build.length?build.join(` `):`Tap words in order…`}),(0,G.jsx)(`div`,{className:`mt-3 flex flex-wrap gap-2`,children:task.tokens.filter((_,ix)=>!build.some((x,j)=>x.__ix===ix&&j>=0)).map((word,ix)=>(0,G.jsx)(a,{variant:`outline`,onClick:()=>{let actual=task.tokens.findIndex((w,j)=>w===word&&!build.some(x=>x.__ix===j));setBuild(v=>[...v,{word,__ix:actual}])},children:word},word+`-`+ix))}),(0,G.jsxs)(`div`,{className:`mt-3 flex gap-2`,children:[(0,G.jsx)(a,{variant:`secondary`,onClick:()=>setBuild([]),children:`Reset`}),(0,G.jsx)(a,{disabled:build.length!==task.tokens.length,onClick:()=>{let value=build.map(x=>x.word).join(` `),ok=norm(value)===norm(task.answer);log(task.skill,ok,`Sentence build`);advance(ok,task.skill,`Sentence build`)},children:`Check sentence`})]})]}):
