(() => {
  ...
  function makeButton(label, action, payload, status, output) {
    ...
    button.addEventListener("click", async () => {
      ...
      try {
        ...
        if (!response.ok) { const detail = String(data?.detail || "").trim(); throw new Error((data?.error || "AI explanation is unavailable right now.") + (detail ? " " + detail : "")); }
  ...
  new MutationObserver(() => {
    ...
    if (current && (!panel || panel.dataset.geminiInline !== "6")) schedule();
