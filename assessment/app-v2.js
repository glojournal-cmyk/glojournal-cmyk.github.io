...
function finish(p,d){clearInterval(timer);timer=null;let earned=0;const rows=d.questions.map((q,i)=>{const answer=d.answers[i]||'',right=correct(q,answer);if(right)earned++;return `<article class="entry"><div><span class="tag">Question ${i+1} · ${right?'Correct':'Incorrect'} · ${esc(q.topic||'')}</span><p>${esc(q.prompt)}</p><p><b>Your answer:</b> ${esc(answer)||'—'}</p><p><b>Model answer:</b> ${esc(q.answer.accepted.join(' / '))}</p>${q.feedback?.short?`<p class="muted">${esc(q.feedback.short)}</p>`:''}</div></article>`});
 const score=Math.round(earned/d.questions.length*100);state.results.push({date:new Date().toLocaleDateString('en-GB'),paper:p.name,correct:earned,total:d.questions.length});state.recent[p.id]=d.questions.map(q=>q.id);delete state.drafts[p.id];save();cards();
 $('#exam').innerHTML=`<h2>${score>=85?'Passed':'Not yet passed'} · ${score}/100</h2><p>${earned}/${d.questions.length} correct. Pass mark: 85/100. Review the answers below, then start a new paper for a different selection.</p><button id="back-to-papers" class="primary">New paper</button><div class="entries">${rows.join('')}</div>`;
